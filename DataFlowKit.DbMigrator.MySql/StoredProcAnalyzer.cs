...
namespace DataFlowKit.DbMigrator.MySql
{
    public class StoredProcAnalyzer
    {
        ...
        public void GenerateClassesFromStoredProc(string storedProcName, string outputPath = "", string namingConvention = "GeneratedModels")
        {
            try
            {
                ...
                var classCode = GenerateClassFile(storedProcName, parameters, resultSets, namingConvention,relativePathOfEntityFolder);
...
ORDER BY ORDINAL_POSITION;", parameters);
        ...
        private string GenerateClassFile(string storedProcName, IEnumerable<ParameterInfo> parameters,
            List<ResultSetInfo> resultSets, string namingConvention,string nameSpaceName)
        {
